...
const ThongKeMuChuMuc1 = () => {
    const [data, setData] = React.useState();
  
  const getThongKeMuChuMuc1s = async () => {
    try {
      const result = await googleAPI.getThongKeMuChuMuc1({ timeout: 5000 }); // 5000ms = 5 seconds
  ...
  const NuDantocMuchuSet15_60Total = data?.reduce((sum, item) => sum + (item.NuDantocMuchuSet15_60 || 0), 0)
    ...
    return <Fragment>
        ...
        <Button variant="contained" onClick={() => HookExportHTMLExcel('my-table-3', 'thong-ke-hien-trang-mu-chu-muc-1')}>Export to Excel</Button >
        <TableContainer
        id="my-table-3"
            component={Paper}
            ...
            key='table 2'
        >
            <Table aria-label="a dense table" size="medium" key='table 1' >
                <TableHead>
                    <TableRow key='a1'>
...
export default ThongKeMuChuMuc1